...
public:
  ...
  bool runOnModule(Module &M) override {
    ...
    Function *main = Function::Create(
        FunctionType::get(i32Ty, params, false),
        GlobalValue::LinkageTypes::ExternalLinkage, "main", &M);

    IRBuilder<> B(ctx);
    B.SetInsertPoint(BasicBlock::Create(ctx, "", main));
    ...
    CallInst *call = B.CreateCall(entry, args);

    Value *isFailure = B.CreateICmpNE(call, ConstantInt::get(i32Ty, 0));
