...
// RUN: %sea kernel --specific-function=qca8k_mdio_register_unsat %t-merged.bc | OutputCheck %s
// CHECK: ^unsat$
...
static int
qca8k_mdio_register_unsat(struct qca8k_priv *priv)
{
...
qca8k_setup_mdio_bus(struct qca8k_priv *priv)
{
	...
	return qca8k_mdio_register_unsat(priv);
}
...
static int
qca8k_setup_unsat(struct dsa_switch *ds)
{
	struct qca8k_priv *priv = ds->priv;
	struct dsa_port *dp;
	int cpu_port, ret;
...
const struct dsa_switch_ops qca8k_switch_ops_unsat = {
	.setup			= qca8k_setup_unsat,
};
